-- Add completion_guidance to supported adaptation types
-- Update constraint to include all generated adaptation types

ALTER TABLE public.adaptations 
DROP CONSTRAINT IF EXISTS adaptations_adaptation_type_check;

ALTER TABLE public.adaptations 
ADD CONSTRAINT adaptations_adaptation_type_check 
CHECK (adaptation_type IN (
    'field_reorder', 
    'field_reordering', 
    'progressive_disclosure', 
    'context_switching', 
    'error_prevention', 
    'completion_guidance',
    'validation_timing', 
    'visual_emphasis'
));

-- Add comment for all adaptation types
COMMENT ON CONSTRAINT adaptations_adaptation_type_check ON public.adaptations 
IS 'Valid adaptation types for comprehensive form optimization including ML-generated types';